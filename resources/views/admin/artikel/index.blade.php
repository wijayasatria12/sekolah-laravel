...
<div class="bg-white/70 backdrop-blur-xl rounded-3xl shadow-2xl p-8">
  <div class="flex items-center justify-between mb-6">
    ...
    <a href="{{ route('admin.artikel.form') }}"
      ...
      <tbody>
        @forelse ($artikels as $index => $p)
          <tr class="bg-white/50 backdrop-blur-md border-b border-gray-200 hover:bg-green-50 transition-all duration-200">
            ...
            <td class="px-6 py-3 font-semibold text-gray-700">{{ $p->judulartikel }}</td>
            ...
            <td class="px-6 py-3 text-gray-700">{{ $p->author }}</td>
            <td class="px-6 py-3 text-gray-700">{{ $p->created_at->format('d-m-Y') }}</td>
            <td class="px-6 py-3 text-center flex justify-center gap-2">
            <a href="{{ route('admin.artikel.edit', $p->id) }}"
            ...
            <form action="{{ route('admin.artikel.destroy', $p->id) }}" method="POST"
        ...
        @empty
          <tr>
            <td colspan="6" class="text-center py-6 text-gray-500">Belum ada data artikel.</td>
          </tr>
        @endforelse
      </tbody>
    </table>
