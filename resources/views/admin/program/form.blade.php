...
<div class="max-w-3xl mx-auto p-8 bg-white rounded-2xl shadow-lg border border-gray-100">
  <div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
      ...
      {{ $program->exists ? 'Edit Program' : 'Tambah Program Baru' }}
    </h1>
  ...
  <form method="POST"
        action="{{ $program->exists ? route('admin.program.update', $program->id) : route('admin.program.store') }}"
    ...
    <!-- Nama Program -->
    <div>
      <label class="block text-sm font-semibold text-gray-600 mb-2">Nama Program</label>
      <input type="text" name="namaprogram"
    ...
    <div>
      ...
      <textarea 
          ...
          placeholder="Tulis deskripsi singkat program..."
          class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">{{ old('deskripsi', $program->deskripsi) }}</textarea>

      <div class="flex justify-between text-xs text-gray-500 mt-1">
        <span>Ketik deskripsi singkat program.</span>
    ...
    <!-- Upload Gambar -->
    <div>
      <label class="block text-sm font-semibold text-gray-600 mb-2">Gambar Program (jpg/png)</label>
